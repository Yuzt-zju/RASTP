...
def format_metric_name(metric: str) -> str:
    """
    将 val/ndcg@5 -> NDCG@5, val/recall@10 -> Recall@10
    """
    # 移除 'val/' 前缀
    name = metric.replace("val/", "")
    # 匹配开头的字母部分（如 ndcg, recall）
    match = re.match(r"([a-zA-Z]+)(.*)", name)
    if match:
        prefix, suffix = match.groups()
        # 特殊处理：ndcg 全大写，recall 首字母大写
        if prefix.lower() == "ndcg":
            prefix = "NDCG"
        else:
            prefix = prefix.capitalize()  # recall -> Recall
...
def plot_comparison(agg_per_mode: Dict[str, pd.DataFrame], metric: str, out_path: str):
    ...
    plt.title(format_metric_name(metric))  # ✅ 使用格式化名称
    leg = plt.legend()
    for text in leg.get_texts():
        text.set_weight('bold')
    plt.tight_layout()
    plt.savefig(out_path, dpi=200, bbox_inches='tight', pad_inches=0.01)
    plt.close()
...
def plot_comparison_grid(agg_per_mode: Dict[str, pd.DataFrame], metrics: List[str], out_path: str):
    ...
    for idx, metric in enumerate(metrics):
        ...
        ax.set_xlabel("Step")
        ax.set_ylabel("")
        ax.set_title(format_metric_name(metric))  # ✅ 使用格式化名称
...
def main():
    ...
    for metric in plot_metrics:
        # ✅ 文件名也使用格式化后的名称（不含特殊字符）
        clean_name = format_metric_name(metric).replace("@", "_at_")
        out_png = os.path.join(OUTPUT_DIR, f"compare_{clean_name}.png")
        plot_comparison(agg_per_mode, metric, out_png)
