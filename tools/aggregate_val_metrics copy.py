...
BASE_DIR = "/data/zhantianyu/Project/GRID/logs/train/runs/sports/gr"
OUTPUT_DIR = os.path.join(BASE_DIR, "aggregates")


MODES = {
    "original": "rkmeans_original_seed_*",
...
def plot_comparison(agg_per_mode: Dict[str, pd.DataFrame], metric: str, out_path: str):
    ...
    for mode, agg in agg_per_mode.items():
        ...
        plt.fill_between(x, y - s, y + s, alpha=0.2, label=mode)
    plt.xlabel("step")
    plt.ylabel(metric)
    plt.title(f"{metric} mean +/- std across seeds")
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    for metric in plot_metrics:
        out_png = os.path.join(OUTPUT_DIR, f"compare_{metric.replace('/', '_')}.png")
